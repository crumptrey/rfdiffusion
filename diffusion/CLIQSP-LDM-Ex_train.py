...
def setup_diffusion_model(config):
    return DiffusionModel(
        ...
        #embedder= None
        embedder= CLIQSP_Embedder_Ex(config)
    )
...
class CLIQSP_Embedder_Ex(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, text_inputs):
        self.CLIQSP.signal_encoder.eval()
        signal_embedding = self.CLIQSP.signal_encoder.encode(text_inputs).to('cuda:0')
        signal_embedding = signal_embedding.view(signal_embedding.size(0), -1)  # Shape: [512, 8192]
        embedding = self.CLIQSP.signal_projection_head(signal_embedding).to('cuda:0')  # Shape: [512, 768]
        return embedding
...
def train_model(model, ae, optimizer, data_loader, accelerator, config):
    ...
    for epoch in range(config['epochs']):
        for x, prompt in data_loader:
            ...
            loss = model(z, text=x, embedding_mask_proba=0.1)
