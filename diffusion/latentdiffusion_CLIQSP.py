...
class ContrastiveModel(nn.Module):
    def __init__(self, config):
        ...
        self.signal_encoder = AutoEncoder1d(
            ...
            patch_size=config['ae_patch_size'], resnet_groups=config['ae_resnet_groups'],
            bottleneck=TanhBottleneck())
...
def setup_diffusion_model(config):
    return DiffusionModel(
        ...
        embedder= None
        #embedder= CLIQSP_Embedder(config)
    )
...
class CLIQSP_Embedder(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')  # Load tokenizer
        self.CLIQSP = torch.load(config['CLIQSP_Path'], map_location='cpu')
        self.embedding_features = config['embedding_features']

    @torch.no_grad()
    def forward(self, text_inputs):
        self.CLIQSP.text_encoder.eval()
        text_inputs = self.tokenizer(text_inputs, padding=True, truncation=True, return_tensors="pt").to("cuda:0")
        text_outputs = self.CLIQSP.text_encoder(**text_inputs)
        embedding = self.CLIQSP.text_projection_head(text_outputs.last_hidden_state[:, 0, :]).to("cuda:0")
