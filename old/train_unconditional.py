...
learning_rate = 1e-4
# Initialize wandb
wandb.init(project='rfdiffusion',
           config = {
...
dataDir = '/home/trey/experiment_rfdiffusion/models/saved_models/'
adam_betas = (0.9, 0.999)
train_modulations = ['AM-SSB', 'CPFSK', 'QPSK', 'GFSK', 'PAM4', 'QAM16', 'WBFM', '8PSK', 'QAM64', 'AM-DSB', 'BPSK']
train_SNRs = np.arange(-20, 19, 2)
model_save_dir = '/home/trey/experiment_rfdiffusion/models/saved_models/'
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (x, mod, snr) in enumerate(data_loader):
        ...
        # Calculate loss
        loss = model(x, text=prompts, embedding_mask_proba=0.1)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
