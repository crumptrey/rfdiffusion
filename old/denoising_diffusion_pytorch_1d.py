...
class ResnetBlock(Module):
    def __init__(self, dim, dim_out, *, time_emb_dim = None, class_emb_dim = None, dropout = 0.):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(int(time_emb_dim) + int(class_emb_dim), dim_out * 2)
        ) if exists(time_emb_dim) or exists(class_emb_dim) else None
...
class Unet1D(Module):
    def __init__(
        ...
        resnet_block = partial(ResnetBlock, time_emb_dim = time_dim, class_emb_dim = classes_dim, dropout = dropout)
...
class GaussianDiffusion1D(Module):
    ...
    def model_predictions(self, x, t,  classes, cond_scale = 6., clip_x_start = False, rederive_pred_noise = False):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, classes, cond_scale = 6., clip_denoised = True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, classes, shape, cond_scale = 6.):
    ...
    @torch.no_grad()
    def ddim_sample(self, classes, shape, cond_scale = 6.,  clip_denoised = True):
    ...
    @torch.no_grad()
    def sample(self, classes, cond_scale = 6., batch_size = 16):
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        loss = self.model(x, classes=mod_class)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            all_samples_list = list(map(lambda n: self.ema.ema_model.sample(batch_size=n, classes = mod_class), batches))
