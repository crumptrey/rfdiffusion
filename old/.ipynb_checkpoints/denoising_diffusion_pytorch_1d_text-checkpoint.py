...
class GaussianDiffusion1D(Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, text, shape, cond_scale = 6.):
    ...
    @torch.no_grad()
    def ddim_sample(self, text, shape, cond_scale = 6.,  clip_denoised = True):
    ...
    @torch.no_grad()
    def sample(self, text, cond_scale = 6., batch_size = 16):
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(text, (batch_size, channels, seq_length), cond_scale)
