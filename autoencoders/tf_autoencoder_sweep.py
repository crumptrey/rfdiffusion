...
from audio_encoders_pytorch import AutoEncoder1d, TanhBottleneck, NoiserBottleneck, STFT
...
def setup_model(config):
    ...
    if config['tf_rep'] == 0:
        rep = STFT(
            num_fft = config["stft_num_fft"],
            hop_length = config["stft_hop_length"],
            window_length = config["stft_window_length"],
            length = config["stft_length"]
        )
    return ae, rep
...
def evaluate_model(model, rep, data_loader, accelerator):
    model.eval()
    total_time_loss = 0.0
    ...
    with torch.no_grad():
        for batch_idx, (x, labels) in enumerate(data_loader):
            x_time = x.to(accelerator.device)
            x_freq = rep.encode(x_time)
            y = model.encode(x_freq)
            y_decoded_freq = model.decode(y)
            y_decoded_time = rep.decode(y_decoded_freq)
            # labels = labels.to(accelerator.device)  # Assuming labels are provided

            # Time-domain reconstruction loss
            time_loss = F.mse_loss(y_decoded_time, x_time)
            total_time_loss += time_loss.item() * x.size(0)

            # Spectrogram-based reconstruction loss
            spectrogram_loss = F.mse_loss(y_decoded_freq, x_freq)
            ...
            if batch_idx < 3:
                for i in range(min(len(x), 1)):
                    plot_waveform_and_spectrogram(x[i], y_decoded_time[i], i)
...
def train_model(model, rep, optimizer, criterion, scheduler, train_loader, val_loader, accelerator, config):
    model, rep, optimizer, train_loader, val_loader, scheduler = accelerator.prepare(
        model, rep, optimizer, train_loader, val_loader, scheduler
    )
    ...
    for epoch in range(config['epochs']):
        for x, _ in train_loader:
            ...
            step += 1
        eval_loss, freq_loss = evaluate_model(model, rep, val_loader, accelerator)
...
def main():
    ...
    train_model(model, rep, optimizer, criterion, scheduler, train_loader, val_loader, accelerator, config)
